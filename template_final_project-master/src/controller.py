...
pygame.init() # initalizations
clock = pygame.time.Clock()
clock.tick(60)  # sets it to 60 fps


screen_width = 800 # sets screen size
screen_height = 800
...
def main():
    game_started = False # variables for different GUI screens
    second_test = False
    respawns = True
    while True: # main loops
        screen.fill('white') # first loop
        if game_started == True: # Black screen with instructions, when space is pressed, next screen
            screen.fill('black')
            all_sprites.add(red, green)
            pass
        else:
            start("Press space to start", 'black', 300, 300, 30)
        if second_test == True: # second screen
        ...
        for event in pygame.event.get(): # main loop
            if event.type == pygame.QUIT: # allows tab to be closed
                pygame.quit()
                sys.exit()
            if event.type == pygame.KEYDOWN: # if space is clicked, the first screen is passed
        ...
        rectangle_1.movement(pygame.K_LEFT, pygame.K_RIGHT) # movement keys
        rectangle_2.movement(pygame.K_a, pygame.K_d)
        pong.movement() # pong movements
        pong.coll(rectangle_1, rectangle_2) # collisions for the two paddles
        all_sprites.draw(screen) # draws the screen
        if respawns == False: # if all respawns are used and green is pressed, ends the game
            if pong.respawns >= 2:
                screen.fill('white')
